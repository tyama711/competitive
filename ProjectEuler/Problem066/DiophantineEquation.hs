...
solve' d = let (k,xs) = solve (BiSurd (d%1) 1 0) []
           in if even k
              then V.toList (getCol 1 (f (take k xs)))
              else V.toList (getCol 1 (f (take (2*k) ((take ((length xs) - k) xs) ++ (cycle (drop ((length xs) - k) xs))))))

f (x:x':xs)
  | xs == [] = (fromList 2 2 [x,1,1,0]) * (fromList 2 2 [x',1,1,0])
  | otherwise = (fromList 2 2 [x,1,1,0]) * f (x':xs)

squares = take 1000 (map (^2) [1..])

main = do
  print $ maximumBy (compare `on` ((!! 0) . snd)) $ map (\d -> (d,solve' d)) ([2..1000] \\ squares)
